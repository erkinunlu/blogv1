...
export default function Sidebar({ categories = [], tags = [] as { id: number; name: string; slug: string }[] }: { categories?: { id: number; name: string; slug: string }[]; tags?: { id: number; name: string; slug: string }[] }) {
	return (
		<aside className="space-y-6">
			<section className="border rounded-lg p-4 bg-white dark:bg-white/5">
			...
			<section className="border rounded-lg p-4 bg-white dark:bg-white/5">
				...
				<div className="flex flex-wrap gap-2">
					{categories.map((c) => (
						<Link key={c.id} href={`/?category=${c.slug}`} className="text-sm px-2 py-1 rounded bg-brand-light dark:bg-white/10 hover:bg-brand-pink/10">
							{c.name}
						</Link>
					))}
				</div>
